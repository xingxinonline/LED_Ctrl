...
unsigned int crc = 0; 
for (i = 0; i < size; i++) {
crc = CRC(crc, databuf[i]); 
} 
return crc;
} 	
	
	
 unsigned int crc16(unsigned char * pszBuf, unsigned int unLength)
{
...
// {
	// unsigned char i,j;
	// unsigned char CRC7_Data = 0;
	// for (i=0;i<len;i++)
	// {
		// CRC7_Data = CRC7_Data^in_data[i];
		// for (j=1;j<=8;j++)
		// {
			// if ((CRC7_Data&0x80)==0x80)
			// {
				// CRC7_Data = (CRC7_Data<<1)^0xE5;
			// }
			// else
				// CRC7_Data = CRC7_Data<<1;
		// }
	// }
	// return CRC7_Data;
// }
